		...
		public function setPrestamo(){
			if($_POST){
				...
				if(empty($_POST['txtIdentificacion']) || empty($_POST['txtMonto']) || empty($_POST['txtInteres']) || empty(['txtCuotas']) || empty(['valor_cuota'])
					...
					$dtFecha         = convertDateFormat(strClean($_POST['fecha_prestamo']));
					$intMonto        = floatval(removerMiles($_POST['txtMonto']));
					$intCuotas       = intval($_POST['txtCuotas']);
					$intInteres      = $_POST['txtInteres'];
					$intValorCuota   = floatval(removerMiles($_POST['valor_cuota']));
					$intTotalInteres = floatval(removerMiles($_POST['valor_interes']));
					$intMontoTotal   = floatval(removerMiles($_POST['valor_total']));
					...
					if($_SESSION['permisosMod']['w']){
						...
						foreach($datos as $row){
							...
							$dtFecha    = strClean($columna2);
							$intCuota   = floatval(removerMiles($columna3));
							$intInteres = floatval(removerMiles($columna4));
							$intCapital = floatval(removerMiles($columna5));
							$intSaldo   = floatval(removerMiles($columna6));
