<?php
	...
	class UsuariosModel extends Mysql
	{
		...
		public function insertUsuario(string $identificacion, string $nombre, string $apellido, string $telefono, string $email, string $password, int $tipoid, int $status)
		{
			...
			if (empty($request)) {
				$query_insert = "INSERT INTO persona(identificacion,nombres,apellidos,telefono,email_user,password,rolid,estado)
								  VALUES(?,?,?,?,?,?,?,?)";
		...
		public function selectUsuarios()
		{
			...
			$sql = "SELECT p.idpersona,p.identificacion,p.nombres,p.apellidos,p.telefono,p.email_user,p.estado,r.idrol,r.nombrerol
					FROM persona p
					INNER JOIN rol r
					ON p.rolid = r.idrol
		...
		public function selectUsuario(int $idpersona)
		{
			...
			$sql = "SELECT p.idpersona,p.identificacion,p.nombres,p.apellidos,p.telefono,p.email_user,p.nit,p.nombrefiscal,p.direccionfiscal,r.idrol,r.nombrerol,p.estado, DATE_FORMAT(p.datecreated, '%d-%m-%Y') as fechaRegistro
					FROM persona p
					INNER JOIN rol r
					ON p.rolid = r.idrol
		...
		public function updateUsuario(int $idUsuario, string $identificacion, string $nombre, string $apellido, string $telefono, string $email, string $password, int
		$tipoid, int $status)
		{
